...
public class VideoScreen {
    ...
    public synchronized void addInfo(VideoInfo info) {
        LOGGER.info("added info: {} {} pathLength={}", info.playerName(), info.name(), info.path() == null ? 0 : info.path().length());
        lock();
        infos.offer(info);
        unlock();
        playNext();
    ...
    public synchronized boolean queueAction(int action, int index) {
        ...
        try {
            ...
            ArrayList<VideoInfo> list = new ArrayList<>(infos);
    ...
    public synchronized void playNext() {
        ...
        nextTask = CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.info("playing info: {} {} pathLength={}", info.playerName(), info.name(), info.path() == null ? 0 : info.path().length());
