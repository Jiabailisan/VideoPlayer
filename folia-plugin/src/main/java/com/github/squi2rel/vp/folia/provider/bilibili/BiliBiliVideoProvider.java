...
import com.github.squi2rel.vp.folia.provider.IProviderSource;
        ...
        return CompletableFuture.supplyAsync(() -> {
            try (HttpClient client = HttpClient.newHttpClient()) {
                ...
                for (int quality : qualities) {
                    options.add(new MediaOption(Integer.toString(quality), qualityName(quality), hasAuthCookie() ? "Bilibili 服务端登录态" : "Bilibili 未登录可用"));
                }
                return options;
            } catch (Exception e) {
                source.reply(e.toString());
