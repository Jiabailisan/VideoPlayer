...
public class VideoArea {
    ...
    public boolean inBounds(Location location) {
        return min.x <= location.getX() && min.y <= location.getY() && min.z <= location.getZ()
                && max.x >= location.getX() && max.y >= location.getY() && max.z >= location.getZ();
    }
    ...
    public synchronized void forEachPlayer(Consumer<UUID> consumer) {
        for (UUID player : players) consumer.accept(player);
    }
